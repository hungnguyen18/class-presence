<script setup lang="ts">
  ...
  function openConfirmDialog({ deviceId, command }: { deviceId: string; command: string }) {
    const device = listDevice.value.find((d) => d.id === deviceId)
    if (!device) {
      return
    }
  ...
  async function confirmAndSendCommand() {
    ...
    await executeSendCommand({ deviceId: confirmDeviceId.value, command: confirmCommand.value })
  }

  async function executeSendCommand({ deviceId, command }: { deviceId: string; command: string }) {
    ...
    snackbar.value = { show: true, text: `${command} command sent. Waiting for device...`, color: 'info' }
  }
  ...
  <AppLayout>
    <v-container fluid class="pa-5 pa-sm-8">
      ...
      <div class="device-summary mb-6 animate-in animate-delay-1">
        ...
        <div class="device-summary__item">
          <span class="device-summary__count device-summary__count--online">{{ onlineCount }}</span>
          <span class="device-summary__label">Online</span>
        </div>
        <div class="device-summary__divider" />
        <div class="device-summary__item">
          <span class="device-summary__count device-summary__count--offline">{{ listDevice.length - onlineCount }}</span>
        ...
        >
          ...
          >
            ...
            <div
              ...
              :class="device.status === EDeviceStatus.ONLINE ? 'device-card__status-bar--online' : 'device-card__status-bar--offline'"
            />

            <div class="device-card__body">
              <!-- Header row -->
              <div class="device-card__header">
                <div class="device-card__icon-wrap" :class="device.status === EDeviceStatus.ONLINE ? 'device-card__icon-wrap--online' : ''">
                  <v-icon size="22" :color="device.status === EDeviceStatus.ONLINE ? 'success' : undefined">mdi-chip</v-icon>
                  <span
                    class="device-card__pulse"
                    :class="{ 'device-card__pulse--active': device.status === EDeviceStatus.ONLINE }"
                  />
                </div>
                <div class="device-card__identity">
                  <div class="device-card__code">{{ device.deviceCode }}</div>
                  <div class="device-card__id">{{ device.id.slice(0, DEVICE_ID_DISPLAY_LENGTH) }}</div>
                </div>
                <v-chip
                  ...
                  :prepend-icon="device.status === EDeviceStatus.ONLINE ? 'mdi-check-circle' : 'mdi-close-circle'"
                >
              ...
              <div class="device-card__info">
                <div class="device-card__info-item">
                  <v-icon size="15" class="device-card__info-icon">mdi-map-marker-outline</v-icon>
                  <div>
                ...
                <div class="device-card__info-item">
                  <v-icon size="15" class="device-card__info-icon">mdi-clock-outline</v-icon>
                  <div>
                    <div class="device-card__info-label">Last Seen</div>
                    <div class="device-card__info-value">{{ formatLastSeen(device.lastSeen) }}</div>
                  </div>
                </div>
                <div class="device-card__info-item">
                  <v-icon size="15" class="device-card__info-icon">mdi-cog-outline</v-icon>
                  <div>
                    <div class="device-card__info-label">Firmware</div>
                    <div class="device-card__info-value">{{ device.firmwareVersion ?? 'N/A' }}</div>
                  </div>
                </div>
                <div class="device-card__info-item">
                  <v-icon size="15" class="device-card__info-icon">mdi-information-outline</v-icon>
                  <div>
                    <div class="device-card__info-label">Description</div>
                    <div class="device-card__info-value">{{ device.description ?? 'No description' }}</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <!-- Empty state -->
      <div v-if="!isLoading && filteredDevices.length === 0 && searchQuery" class="text-center py-12">
        <v-icon size="48" color="medium-emphasis" class="mb-4">mdi-magnify-close</v-icon>
        <p class="text-body-1 text-medium-emphasis">No devices match "{{ searchQuery }}"</p>
      </div>
...
@keyframes pulse-dot {
  0%, 100% { box-shadow: 0 0 0 2px var(--color-surface), 0 0 0 4px transparent; }
  50% { box-shadow: 0 0 0 2px var(--color-surface), 0 0 0 6px rgba(61, 139, 94, 0.25); }
}
...
@keyframes card-highlight {
  0% {
    box-shadow: 0 0 0 3px var(--color-accent-gold), 0 4px 20px rgba(212, 168, 83, 0.35);
  }
  100% {
    box-shadow: var(--shadow-card);
  }
