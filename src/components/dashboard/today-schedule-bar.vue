        ...
        <div class="d-flex align-center ga-3 pa-3 px-4">
          ...
          <div class="chip-status">
            <template v-if="session.isLive">
              <span class="live-dot" />
              <span class="text-success chip-status-text">{{ session.checkedInCount }}/{{ session.studentCount }}</span>
            </template>
            <template v-else-if="session.isPast">
              <v-icon size="14" color="success" class="mr-1">mdi-check</v-icon>
              <span class="chip-status-text text-medium-emphasis">{{ session.checkedInCount }}/{{ session.studentCount }}</span>
            </template>
            <template v-else>
              <span class="chip-status-text text-medium-emphasis">{{ session.studentCount }} enrolled</span>
            </template>
          </div>
        </div>
      </v-card>
